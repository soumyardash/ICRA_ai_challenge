...
void NodePID::messageCallback(const nav_msgs::Odometry::ConstPtr& msg)
{
  ...
  ROS_INFO("speedcommand: %lf & anglecommand: %lf\n",fmin(maxSpeed,speedCommand),fmin(maxASpeed,angleCommand));
...
bool NodePID::closeEnough(MyPoint* actual)
{
  ...
  //ROS_INFO("c1: %d c2: %d c3:%d\n",fabs(targetAngle - (actual->angle - start->angle)) > toleranceAngle,fabs(targetAngle - (actual->angle - start->angle) + 2*PI) > toleranceAngle,fabs(targetAngle - (actual->angle - start->angle) - 2*PI) > toleranceAngle);
  //ROS_INFO("fabs(targetAngle - (actual->angle - start->angle)): %lf & targetAngle: %lf \n",fabs(targetAngle - (actual->angle - start->angle)),targetAngle);
